function [functionValue equalityConstrVals inequalityConstrVals]=llTestProblem(llPop, testProblemName, ulMember)
    ...
    % 该函数用于计算给定上层成员对应的下层成员集合的目标函数值和约束
    % 输入:
    %   llPop: 下层成员集合 (每一行代表一个成员)
    %   testProblemName: 要求解的测试问题的名称
    %   ulMember: 上层成员 (可以是单个或多个成员)
    % 输出:
    %   functionValue: 下层目标函数值
    %   equalityConstrVals: 等式约束值
    %   inequalityConstrVals: 不等式约束值
    ...
    global llFunctionEvaluations; % 全局变量，用于记录下层目标函数评估的次数
    noOfMembers = size(llPop,1); % 获取下层成员的数量
    llFunctionEvaluations = llFunctionEvaluations + noOfMembers; % 更新目标函数评估次数
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    %Function call here
    ...
    for i=1:noOfMembers  % 遍历每个下层成员进行计算
        if size(llPop,1) == size(ulMember,1) % 如果下层和上层成员数量相同
            [functionValue(i,:) equalityConstrValsTemp inequalityConstrValsTemp] = fhandle(ulMember(i,:), llPop(i,:)); %调用问题句柄，如@smd12，选择 ulMember 的第一行，以及llpop的第一行，llpop和ulmember是一个解的矩阵，进去才开始拆分上下层变量
        elseif size(ulMember,1) == 1  % 如果上层成员只有一个
            [functionValue(i,:) equalityConstrValsTemp inequalityConstrValsTemp] = fhandle(ulMember, llPop(i,:));
        else  % 上层和下层成员数量不匹配，显示错误信息
            disp('Error in llTestProblem, size of llPop and ulMember mismatch');
        end
        
        if ~isempty(equalityConstrValsTemp)  % 如果存在等式约束值，则将其存储
            equalityConstrVals(i,:) = equalityConstrValsTemp;
        end
        if ~isempty(inequalityConstrValsTemp) % 如果存在不等式约束值，则将其存储
